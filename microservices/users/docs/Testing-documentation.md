...
0. ```Register some new users (exactly as has been done above):```\
   ...
   ```Lecturer with username apanichella (he will be registered as student but how to change role is shown in change role section).```\
...
- *Change role*:
0. ```Suppose we register the lecturer from getByRole section. Initially everyone is a student. We want to change the role.```\
   ![new lecturer](images/getByRole/RegisterUsers/registerLecturer.jpg)\
