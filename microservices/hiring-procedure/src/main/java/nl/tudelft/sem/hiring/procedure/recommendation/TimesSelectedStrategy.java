           ...
           this.webClient
            ...
            .flatMap(response -> {
                ...
                return response
                        ...
                        .flatMap(res -> {
                            try {
                                String jsonList = mapper.readTree(res)
                                                    .get("courseIds").toString();
                                List<Long> courseIds = mapper.readValue(jsonList,
                                                    new TypeReference<List<Long>>() {});
                                List<Object[]> l = this.repo
                                ...
                                return Mono.just(
                                        l
                                                .stream()
                                                .limit(number)
                                                .map(a -> new Recommendation((Long) a[0],
                                                                ((Long) a[1]).doubleValue()))
